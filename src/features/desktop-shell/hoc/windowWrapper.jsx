/**
 * PURPOSE:
 * Wrap desktop app content in managed desktop window behavior.
 * RESPONSIBILITY:
 * Apply position, size, focus, drag, maximize, and restore behavior to a window component.
 * USED BY:
 * Desktop app windows.
 * DEPENDS ON:
 * Desktop window store, desktop window config, GSAP Draggable, and clsx.
 * SHOULD NOT HANDLE:
 * App-specific content, portfolio data, mobile layout, or dock rendering.
 * SCALING NOTES:
 * Keep generic window mechanics here. Move app-specific window behavior into the app feature.
 */
...
import { DEFAULT_WINDOW_SIZES } from "../config/windowConfig";
    ...
    useLayoutEffect(() => {
      ...
      if (prevMode !== sizeMode) {
        ...
        if (sizeMode === "normal") {
          const restoreSize =
            originalSizeRef.current ||
            DEFAULT_WINDOW_SIZES[windowKey] || { width: 800, height: 600 };

          const restorePos = originalPositionRef.current || { x: 0, y: 0 };
        ...
        if (sizeMode === "normal") {
          gsap.set(el, {
            ...
            height:
              size?.height || DEFAULT_WINDOW_SIZES[windowKey]?.height || 600,
          });
        }
